...
@router.post("/{wallet_id}/deposit", response_model=WalletResponse, summary="Deposit funds", description="Add funds to a specific wallet. This is recorded as a formal Transaction with an idempotency key.")
async def deposit_funds(wallet_id: uuid.UUID, deposit_in: WalletDeposit, db: DBDep, user: CurrentUser):
    wallet_service = WalletService(db)
    return await wallet_service.deposit(wallet_id, user, deposit_in)
